...
```{r, warning=FALSE}
spotify_data %>% ggplot(aes(x = liveness, y = log(rank))) + geom_line() + geom_smooth(method=lm)
```
...
```{r, warning=FALSE}
spotify_data %>% ggplot(aes(x = log(danceability), y = log(valence))) + geom_line() + geom_smooth(method=lm)
```
